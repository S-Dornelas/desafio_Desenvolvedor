    ...
    public class DepartamentosDAL : BaseDAL
    {
                ...
                using (SQLiteCommand dbCommand = dbConnection.CreateCommand())
                {
                    ...
                    using (SQLiteDataReader dataReader = dbCommand.ExecuteReader())
                    {
                        ...
                        while (dataReader.Read())
                            lstDepartamentos.Add(PreencherDepartamento(dataReader));
        ...
        public Departamento ObterDepartamento(int idDepartamento)
        {
            ...
            using (SQLiteConnection dbConnection = new SQLiteConnection(CONNECTION_STRING))
            {
                using (SQLiteCommand dbCommand = dbConnection.CreateCommand())
                {
                    dbCommand.CommandText = $"SELECT * FROM departamentos WHERE departamentos.ID = {idDepartamento}";
                    ...
                    {
                        if (dataReader.Read())
                            departamento = PreencherDepartamento(dataReader);
                        dataReader.Close();
                    }
                    dbConnection.Close();
                }
        ...
        private Departamento PreencherDepartamento(SQLiteDataReader dataReader)
        {
